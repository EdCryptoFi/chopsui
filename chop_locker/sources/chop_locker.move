module chop_locker::chop_locker {
    ...
    public struct PoolConfig<phantom T0> has key {
        ...
        current_apy: u64,
        apy_daily_increment: u64, //basis points percentage increase
    ...
    public fun new_pool_config<T0>(_admin: &ChopLockerAdmin, coin: Coin<T0>, base_apy: u64, /*lock_period: u64,*/ adi: u64, ctx: &mut TxContext) {
        let balance = coin::into_balance(coin);
        let pool_config = PoolConfig<T0> {
            id: object::new(ctx),
            base_apy: base_apy, // 1000 = Initial base APY: 10.00%
            current_apy: base_apy,
            apy_daily_increment: adi,
    ...
    public fun increase_by_basis_points(amount: u64, basis_points: u64): u64 { //500 = 5%
        let increase = (amount * basis_points)/10000; //basis points for 100
        amount + increase
    }
